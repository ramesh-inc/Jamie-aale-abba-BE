...
const VideoStoryModal: React.FC<VideoStoryModalProps> = ({
  isOpen,
  onClose,
  onStoryCreated
}) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (files.length === 0) {
      newErrors.files = 'At least one video is required';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      submitData.append('story_type', 'video');
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Enter video story title"
            maxLength={100}
          />
          {errors.title && (
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
          ...
          <button
            ...
            className="px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center"
          >
            {isLoading ? (
              <>
