...
interface TeacherLearningActivitiesChartProps {
  selectedStudentId: number;
}

const TeacherLearningActivitiesChart: React.FC<TeacherLearningActivitiesChartProps> = ({ 
  selectedStudentId
}) => {
  ...
  const fetchLearningActivities = async () => {
    ...
    try {
      const response = await teacherApi.getStudentLearningActivities(selectedStudentId, selectedYear);
      setChartData(response);
    } catch (error: any) {
      console.error('Error fetching learning activities:', error);
      
      if (error?.response?.status === 404) {
        setError('Student not found or no learning activities data available');
        setChartData([]);
        ...
        setError('Failed to load learning activities data');
        setChartData([]);
      }
    } finally {
      setLoading(false);
                        ...
                        {data.hours > 0 && (
                          ...
                          >
                            ...
                            <div
                              ...
                              title={`${data.month}: ${data.hours} hours`}
                            >
                              {/* Tooltip */}
...
export default TeacherLearningActivitiesChart;